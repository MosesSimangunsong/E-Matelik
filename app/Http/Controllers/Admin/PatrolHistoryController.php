...
class PatrolHistoryController extends Controller
{
    public function index(): Response
    {
        ...
        $dates = $baseDates
            ->merge(
                $logs->map(fn ($log) => optional($log->patrol_date)->toDateString())->filter()
            )
            ->merge(
                $reports->map(fn ($report) => optional($report->submitted_at ?? $report->created_at)->toDateString())->filter()
            )
            ->unique()
            ->sortDesc()
            ->values();
        ...
        return Inertia::render('Admin/PatrolHistory/Index', [
